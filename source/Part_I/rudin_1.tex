...
\begin{proof}
    ...
    故\(\exists N_1,N_2 \in \mathbb{Z}^+, \forall n>N_1, d(\varphi(x_n),[\{x_n\}])<\varepsilon_0/3, \forall n>N_2, d(\varphi(y_n),[\{y_n\}])<\varepsilon_0/3\).

    令\(N=\max\{N_0,N_1,N_2\}+1, q=(x_N+y_N)/2\)，下证\([\{x_n\}]<\varphi(q)<[\{y_n\}]\).

    由于\(N>N_1, d(\varphi(x_N),[\{x_n\}])<\varphi(\varepsilon_0/3)\)，即\(\varphi(x_N)-[\{x_n\}]>-\varphi(\varepsilon_0/3)\).

    同时\(N>N_0, \varphi(q)-\varphi(x_N)=\varphi((x_N+y_N)/2-x_N)=\varphi((y_N-x_N)/2)>\varphi(\varepsilon_0/2)\).

    因此\(\varphi(q)-[\{x_n\}]=(\varphi(q)-\varphi(x_N))+(\varphi(x_N)-[\{x_n\}])>\varphi(\varepsilon_0/6)\)，即\(\varphi(q)>[\{x_n\}]\).

    由于\(N>N_1, d(\varphi(y_N),[\{y_n\}])<\varphi(\varepsilon_0/3)\)，即\([\{y_n\}]-\varphi(y_N)>-\varphi(\varepsilon_0/3)\).

    同时\(N>N_0, \varphi(y_N)-\varphi(q)=\varphi(y_N-(x_N+y_N)/2)=\varphi((y_N-x_N)/2)>\varphi(\varepsilon_0/2)\).

    因此\([\{y_n\}]-\varphi(q)=([\{y_n\}]-\varphi(y_N))+(\varphi(y_N)-\varphi(q))>\varphi(\varepsilon_0/6)\)，即\([\{y_n\}]>\varphi(q)\).
\end{proof}
